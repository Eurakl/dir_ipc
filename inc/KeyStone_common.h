...
#define PLLCTL0_PLLD_MASK			0x0000003F
#define PLLCTL0_PLLM_SHIFT			6
#define PLLCTL0_PLLM_MASK			0x0007FFC0
#define PLLCTL_OD_SHIFT				19
#define PLLCTL_OD_MASK				0x00780000
#define PLLCTL_BYPASS_SHIFT			23
#define PLLCTL_BYPASS_MASK			0x00800000
#define PLLCTL0_BWADJ_SHIFT			24
#define PLLCTL0_BWADJ_MASK			0xFF000000

#define PLLCTL1_BWADJ_SHIFT			0
#define PLLCTL1_BWADJ_MASK			0x0000000F
#define PLLCTL1_ENSAT_SHIFT			6
#define PLLCTL1_ENSAT_MASK			0x00000040
#define PLLCTL1_PAPLL_SHIFT			13
#define PLLCTL1_PAPLL_MASK			0x00002000
#define PLLCTL1_PLLRESET_SHIFT		14
#define PLLCTL1_PLLRESET_MASK		0x00004000

/*----------------------Boot Config registers definition----------------*/
#define DEVSPEED					*(unsigned int *)0x026203F8

#define DEVSPEED_1000_MHZ_MASK_L	0x00800000
#define DEVSPEED_1000_MHZ_MASK_H	0x40000000
#define DEVSPEED_1200_MHZ_MASK_L	0x01000000
#define DEVSPEED_1200_MHZ_MASK_H	0x20000000
#define DEVSPEED_1250_MHZ_MASK_L	0x02000000
#define DEVSPEED_1250_MHZ_MASK_H	0x10000000
#define DEVSPEED_1400_MHZ_MASK_L	0x04000000
#define DEVSPEED_1400_MHZ_MASK_H	0x08000000
#define DEVSPEED_800_MHZ_MASK_H		0x80000000

#define DEVSTAT_LENDIAN_SHIFT		0
#define DEVSTAT_LENDIAN_MASK		0x00000001
#define DEVSTAT_BOOTMODE_SHIFT		1
#define DEVSTAT_BOOTMODE_MASK		0x0000000E
#define DEVSTAT_SYSPLL_SHIFT		11
#define DEVSTAT_SYSPLL_MASK			0x00003800
#define DEVSTAT_BOOTMASTER_SHIFT	14
#define DEVSTAT_BOOTMASTER_MASK		0x00004000
...
#define TMR_TCR_READRSTMODE_LO_MASK		0x00000400
#define TMR_TCR_CAPMODE_LO_SHIFT		11
#define TMR_TCR_CAPMODE_LO_MASK			0x00000800
#define TMR_TCR_CAPEVTMODE_LO_SHIFT		12
#define TMR_TCR_CAPEVTMODE_LO_MASK		0x00003000
#define TMR_TCR_READRSTMODE_HI_SHIFT	26
#define TMR_TCR_READRSTMODE_HI_MASK		0x04000000

#define TMR_TGCR_PLUSEN_SHIFT			4
#define TMR_TGCR_PLUSEN_MASK			0x00000010

#define TMR_INTCTLSTAT_EN_ALL_CLR_ALL	0x000F000F

//#define CSL_TMR_WDTCR_WDKEY_CMD1		0x0000A5C6
//#define CSL_TMR_WDTCR_WDKEY_CMD2		0x0000DA7E
...
#define GLOBAL_ADDR(addr)	((unsigned int)addr<0x1000000?\
							(unsigned int)addr+(0x10000000+DNUM*0x1000000):\
							(unsigned int)addr)
...
#define DDR_PHY_ADDR(addr) ((((unsigned int)addr & 0x7FFFFFFF) >> 4) | 0x80000000)

/* max/min number in two numbers */
...
typedef struct
{
	volatile unsigned int	PLL_CTL0;
	volatile unsigned int	PLL_CTL1;
}PLL_ControlRegs;
...
extern void TSC_delay_ms(unsigned int ms);

/*delay in microsecond*/
extern void TSC_delay_us(unsigned int us);
...
typedef struct{
	unsigned int		timer_num;		/* select one timer */
...
extern void	Reset_Timer(unsigned int timer_num);
...
extern void Service_Watchdog(unsigned int timer_num);

/*===============================PSC===================================*/
extern int KeyStone_enable_PSC_module(unsigned int pwrDmnNum, unsigned int moduleNum);
extern int KeyStone_disable_PSC_module(unsigned int pwrDmnNum, unsigned int moduleNum);
extern int KeyStone_disable_PSC_Power_Domain(unsigned int pwrDmnNum);
...
typedef struct
{
	unsigned char CC_num; 		//number of the CC for the channel
	unsigned char channel_num;
	unsigned char TC_num; 		//number of the TC used for the channel
}EDMA_CC_Chanel_TC;

/* setup uiChannel of an EDMA to use uiTC */
extern void EDMA_channel_TC_cfg(unsigned int uiCC,
		unsigned int uiChannel, unsigned int uiTC);
extern void EDMA_TC_priority_cfg(unsigned int uiCC,
		unsigned int uiPri, unsigned int uiTC);
extern void EDMA_init(void);

extern void EDMA_event_enable(unsigned int uiCC, unsigned int uiChannel);
extern void EDMA_event_disable(unsigned int uiCC, unsigned int uiChannel);
extern void EDMA_interrupt_enable(unsigned int uiCC, unsigned int uiIntNum);
extern void EDMA_interrupt_disable(unsigned int uiCC, unsigned int uiIntNum);
...
extern void lock_mem_prot_regs(void);
extern void unlock_mem_prot_regs(void);
/* setup the memory protection for local L1 memory */
extern void L1_MPPA_setup(volatile unsigned int MPPA_regs[], unsigned int MPPA[]);
/* config "AccessPermisionMask" for L1P */
extern void L1P_memory_protection_cfg(unsigned int AccessPermisionMask);
/* config "AccessPermisionMask" for L1D */
extern void L1D_memory_protection_cfg(unsigned int AccessPermisionMask);
/* -----protect L1 as cache ---- */
 ...
 *     LL2_MPPA_setup(MPPA);
 */
extern void LL2_MPPA_setup(unsigned int MPPA[]);
/* config "AccessPermisionMask" for local L2 memory section from
 * "startAddress" with "byteCount" size */
extern void LL2_Memory_Protection_cfg(unsigned int startAddress,
...
extern void KeyStone_CorePac_AID_map(unsigned char map_table[]);
...
typedef struct
{
	volatile unsigned int MPAXL;
	volatile unsigned int MPAXH;
}MPAX_Regs;

/* memory protection and address extension configuration */
typedef struct
{
	unsigned int BADDR;					/* 32-bit virtual base address */
	unsigned int RADDR;					/* 36-bit physical replace address, right shift by 4 to get the value here) */
	unsigned int SegementSize;			/* Segment size in byte, must be power of 2 */
	unsigned int AccessPermisionMask;	/* Access types allowed in this address range */
}MPAX_Config;

/* configure one MPAX segment */
extern void KeyStone_MPAX_seg_setup(MPAX_Regs *MPAX_regs, unsigned int BADDR,
...
extern void KeyStone_MSMC_MP_interrupt_en(unsigned int PRIVID_MASK);

/*==========Peripherals registers and internal data buffer MPU=============*/
/* peripherals memory protection unit configuration */
typedef struct
{
	unsigned int StartAddr;				/* 32-bit start address */
	unsigned int EndAddr;				/* 32-bit end address */
	unsigned int AccessPermisionMask;	/* Access types allowed in this address range */
}MPU_Range_Config;
...
extern void KeyStone_MPU_setup(unsigned int uiMPU_num,
...
extern void KeyStone_MPU_MPPA_modify(unsigned int uiMPU_num,
		unsigned int uiAddress, unsigned int AccessPermisionMask);

/* enable one MPU interrupt */
extern void KeyStone_MPU_interrupt_enable(unsigned int uiMPU_num);
/* enable all MPU interrupts */
extern void KeyStone_MPU_interrupts_enable(void);

/*======================memory EDC=====================================*/
/* Enable MSMC EDC and setup scrubbing cycle counter */
extern void KeyStone_SL2_EDC_enable(unsigned int scrubCnt);

/* Enable MSMC EDC error interrupt */
extern void KeyStone_SL2_EDC_interrupt_en(void);

/* enable LL2 EDC and scrub whole LL2 */
extern void LL2_EDC_setup(void);

/* L1P EDC setup */
extern void L1P_EDC_setup(void);
...
extern void LL2_EDC_scrub(unsigned int uiByteCnt);

/* LL2 EDC exception/interrupt handler */
extern void LL2_EDC_handler(Bool bCorrected);

/* SL2 EDC handler */
extern void KeyStone_SL2_EDC_handler(void);

/*===============================Exception=============================*/
typedef union
{
	volatile unsigned int MPFSR;				/* copy of MPFSR register for L1P, L1D or LL2 */
	volatile unsigned int SMPFR;				/* copy of SMPFR register for MSMC */
	volatile unsigned int MPU_FAULT_STATUS;		/* copy of FAULT_STATUS register for MPU */
	volatile unsigned int L1PEDSTAT;			/* copy of L1PEDSTAT register */
	volatile unsigned int L2EDSTAT;				/* copy of L2EDSTAT register */
	volatile unsigned int SMESTAT;				/* copy of SMESTAT register for SL2 EDC */
	volatile unsigned int DDR_IRQSTATUS_SYS;	/* copy of IRQSTATUS_SYS for DDR ECC */
}Memory_Fault_Info;
...
typedef	struct
{
	...
	volatile unsigned int	BUS_ERROR;			/* copy of MDMAERR or ECFGERR register */
} Memory_Exception_Info;

typedef struct
{
	volatile unsigned int EMR;					/* copy of TPTC_EMR register */
	volatile unsigned int EMRH;					/* copy of TPTC_EMRH register */
	volatile unsigned int QEMR;					/* copy of TPTC_QEMR register */
	volatile unsigned int CCERR;				/* copy of TPTC_CCERR register */
}EDMA_CC_Exception_Info;

typedef struct
{
	volatile unsigned int ERRSTAT;				/* copy of TPTC_ERRSTAT register */
	volatile unsigned int ERRDET;				/* copy of TPTC_ERRDET register */
...
typedef union
{
	volatile unsigned int	INTXSTAT;			/* copy of INTXSTAT register */
...
typedef struct
{
	volatile unsigned int	MEXPFLAG[4];		/* copy of the MEXPFLAG0..3 registers */
	volatile unsigned int	CIC_STATUS[7];		/* copy of the CIC status registers */
	Exception_Info			info;
}External_Exception_Status;

typedef union
{
	volatile unsigned int		IERR;			/* copy of the IERR register */
...
typedef struct
{
	volatile unsigned int	B3;		/* copy of B3 register (return pointer of caller) */
	volatile unsigned int	A4;		/* copy of the A4 register (first input parameter of caller) */
	volatile unsigned int	B4;		/* copy of the B4 register (second input parameter of caller) */
	volatile unsigned int	B14;	/* copy of the B14 register (data pointer) */
	volatile unsigned int	B15;	/* copy of the B15 register (stack pointer) */
	volatile unsigned int	TSCL;	/* copy of the TSCL register (time stamp) */
	volatile unsigned int	TSCH;	/* copy of the TSCH register (time stamp) */
	volatile unsigned int	NTSR;	/* copy of the NTSR register */
	volatile unsigned int	NRP;	/* copy of the NRP register */
	volatile unsigned int	EFR;	/* copy of the EFR register */
	volatile unsigned int	ITSR;	/* copy of the ITSR register */
	volatile unsigned int	IRP;	/* copy of the IRP register */
...
/* INTC event number mapping to CIC exception events */
extern unsigned int guwIntcCICExcEvtIdx;
/* CIC output event number */
extern unsigned int gCIC_EXC_out_num;
...
static inline void InvalidCache(void * blockPtr, unsigned int byteCnt)
{
	...
	if((unsigned int)blockPtr >= 0x0C000000 && (unsigned int)blockPtr < 0x0D000000)
	{
		CACHE_invL1d((void *)blockPtr, byteCnt, CACHE_WAIT);
		CSL_XMC_invalidatePrefetchBuffer();
	}
	else if((unsigned int)blockPtr >= 0x10000000)
	{
...
static inline void WritebackCache(void *blockPtr, unsigned int byteCnt)
{
	if(byteCnt>0xFFFF*4)
	{
		byteCnt= 0xFFFF*4;
...
{
	if((unsigned int)blockPtr >= 0x0C000000 && (unsigned int)blockPtr < 0x0D000000)	//SL2
	{
		CACHE_wbAllL1d(CACHE_WAIT);;
	}
	else if((unsigned int)blockPtr>=0x10000000) 	//DDR or other cores
	{
...
static inline void WritebackInvalidCache(void * blockPtr, unsigned int byteCnt)
{
#if CACHE_DISABLE_INT
/*workaround for the "L2 Cache Corruption During Block and Global Coherence Operations
Issue" described in errata of some devices*/
...
static inline void KeyStone_CIC_clear_system_event(CSL_CPINTCRegs *cpIntcRegs,
			int input_event_num)
{
	volatile unsigned int dummy_value;
...
static inline TDSP_Board_Type KeyStone_Get_dsp_board_type(void)
{
	unsigned int deviceID = gpBootCfgRegs->DEVICE_ID_REG0 & 0x0FFFFFFF;

	gDSP_board_type = UNKNOWN;
	if(deviceID == 0x0009D02F)
	{
		gDSP_board_type= DUAL_NYQUIST_EVM;
	}
	if(deviceID == 0x0B94102F)
	{
		gDSP_board_type = C6670_EVM;
	}
	if(deviceID == 0x0009E02F)
	{
		gDSP_board_type = C6678_EVM;
	}
	if(deviceID == 0x0B96202F)
	{
		gDSP_board_type = TCI6614_EVM;
	}
	if(deviceID == 0x0B97A02F)
	{
...
static inline unsigned int KeyStone_Get_DSP_Number(void)
{
