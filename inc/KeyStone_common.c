...
unsigned int gDSP_Core_Speed_Hz = 1000000000;
...
/*****************************************************************************
 ...
 Description  : DSP core PLL configuration
                DSP core will be configured to run at
...
*****************************************************************************/
void KeyStone_main_PLL_init(float ref_clock_MHz, unsigned int multiplier, unsigned int divisor)
{
	unsigned int i;
	 ...
	 * execute following steps if BYPASS == 1 (if bypass enabled) */
	if(gpPLLC_regs->SECCTL & PLLCTL_BYPASS_MASK)
	{
		/* a. In MAINPLLCTL1, write ENSAT = 1 (for optimal PLL operation) */
		gpBootCfgRegs->CORE_PLL_CTL1 |= PLLCTL1_ENSAT_MASK;
		...
		gpPLLC_regs->PLLCTL |= CSL_PLLC_PLLCTL_PLLPWRDN_MASK;

		/* g. Wait for at least 5 ¦Ìs based on the reference clock CLKIN
		 * (PLL power down toggling time) */
		for(i=0; i< 1000; i++)
			asm(" nop 5");
	...
	else
	{
		...
		/* c. Wait 4 cycles of the reference clock CLKIN
		 * (to make sure the PLL controller mux switches properly to the bypass) */
	...
	gpBootCfgRegs->CORE_PLL_CTL0 =
			((multiplier-1) << PLLCTL0_BWADJ_SHIFT) |
			(((multiplier*2-1) & 0x1FC0) << PLLCTL0_PLLM_SHIFT) |
			(divisor-1);

	gpPLLC_regs->PLLM = (multiplier*2-1) & 0x3F;
	gpBootCfgRegs->CORE_PLL_CTL1 &= ~PLLCTL1_BWADJ_MASK;
	gpBootCfgRegs->CORE_PLL_CTL1 |= (multiplier-1) >> 8;
	...
	gpPLLC_regs->PLLDIV1_3[3-1] = (3-1) | CSL_PLLC_PLLDIV1_3_DNEN_MASK;
	gpPLLC_regs->PLLDIV4_16[4-4] = (5-1) | CSL_PLLC_PLLDIV4_16_DNEN_MASK;
	gpPLLC_regs->PLLDIV4_16[7-4] = (6-1) | CSL_PLLC_PLLDIV4_16_DNEN_MASK;
...
/*****************************************************************************
 ...
 Description  : Config the PLL of PA and DDR
                target clock speed will be
                ref_clock_MHz/inputDivisor*multiplier/outputDivisor
		...
		unsigned int multiplier, unsigned int outputDivisor)
{
	...
	PLL_Regs->PLL_CTL1 |= (multiplier/2-1) >> 8;

	/* 5. In PLLCTL1, write PLLRST = 1 (PLL is reset) */
	PLL_Regs->PLL_CTL1 |= PLLCTL1_PLLRESET_MASK;
...
/*****************************************************************************
 ...
 Return Value :
  ...
  1.Date         : 2013-2-14
    ...
    Modification : Created function
*****************************************************************************/
void KeyStone_PASS_PLL_init(float ref_clock_MHz, unsigned int multiplier, unsigned int divisor)
{
	/* output divisor for PASS PLL should be 2 */
	if(KeyStone_PLL_init((PLL_ControlRegs *)&gpBootCfgRegs->PA_PLL_CTL0,
			divisor, multiplier*2, 2) == 0)
	{
...
*****************************************************************************/
void KeyStone_DDR_PLL_init(float ref_clock_MHz, unsigned int multiplier, unsigned int divisor)
{
	/* output divisor for DDR PLL should be 2 */
	if(KeyStone_PLL_init((PLL_ControlRegs *)&gpBootCfgRegs->DDR3_PLL_CTL0,
			divisor, multiplier, 2) == 0)
	{
...
*****************************************************************************/
void TSC_delay_ms(unsigned int ms)
{
...
/*****************************************************************************
 ...
 Return Value :
*****************************************************************************/
void TSC_delay_us(unsigned int us)
{
	volatile unsigned long long startTSC, currentTSC;
	unsigned long long delay_cycles;
	unsigned int tscl, tsch;
...
/*****************************************************************************
 ...
 Return Value :
  ...
  1.Date         : 2010/12/12
    ...
    Modification : Created function
*****************************************************************************/
void Reset_Timer(unsigned int timer_num)
{
...
void Timer64_Init(Timer64_Config *tmrCfg)
{
	...
	if(tmrCfg->timerMode == TIMER_PERIODIC_CLOCK)
	{
	...
	if(tmrCfg->timerMode == TIMER_WATCH_DOG)
	{
		gpTimerRegs[tmrCfg->timer_num]->TGCR =
				/* Select watchdog timer mode */
	...
	else if(tmrCfg->timerMode == TIMER_PERIODIC_WAVE)
	{
	...
	else if(tmrCfg->timerMode == TIMER_ONE_SHOT_PULSE)
	{
		gpTimerRegs[tmrCfg->timer_num]->TCR =
				(CSL_TMR_CLOCK_INP_NOGATE << CSL_TMR_TCR_TIEN_LO_SHIFT) |
				(CSL_TMR_CLKSRC_INTERNAL << CSL_TMR_TCR_CLKSRC_LO_SHIFT) |
	...
	else if(tmrCfg->timerMode == TIMER_PERIODIC_CLOCK)
	{
		gpTimerRegs[tmrCfg->timer_num]->TCR =
				(CSL_TMR_CLOCK_INP_NOGATE << CSL_TMR_TCR_TIEN_LO_SHIFT) |
				(CSL_TMR_CLKSRC_INTERNAL << CSL_TMR_TCR_CLKSRC_LO_SHIFT) |
	...
	else if(tmrCfg->timerMode == TIMER_PERIODIC_PULSE)
	{
		gpTimerRegs[tmrCfg->timer_num]->TCR =
				(CSL_TMR_CLOCK_INP_NOGATE << CSL_TMR_TCR_TIEN_LO_SHIFT) |
				(CSL_TMR_CLKSRC_INTERNAL << CSL_TMR_TCR_CLKSRC_LO_SHIFT) |
...
/*****************************************************************************
 ...
 Return Value :
  ...
  1.Date         : 2010/12/12
    ...
    Modification : Created function
*****************************************************************************/
void Service_Watchdog(unsigned int timer_num)
{
...
/*============================EDMA=====================================*/
 ...
 Return Value :
  ...
  1.Date         : 2010/12/12
    ...
    Modification : Created function
*****************************************************************************/
void EDMA_channel_TC_cfg(unsigned int uiCC, unsigned int uiChannel, unsigned int uiTC)
{
...
/*****************************************************************************
 ...
 Return Value :
  ...
  1.Date         : 2010/12/12
    ...
    Modification : Created function
*****************************************************************************/
void EDMA_TC_priority_cfg(unsigned int uiCC, unsigned int uiPri, unsigned int uiTC)
{
...
void EDMA_init(void)
{
	...
	for(i=0;i<CSL_EDMA3_TPCC0_NUM_PARAMSETS*8;i++)
	{
		*uipPaRAM++ = 0;
	}

	uipPaRAM = (unsigned int *)&(gpEDMA_CC1_regs->PARAMSET[0]);
	for(i=0;i<CSL_EDMA3_TPCC1_NUM_PARAMSETS*8;i++)
	{
		*uipPaRAM++ = 0;
	}

	uipPaRAM = (unsigned int *)&(gpEDMA_CC2_regs->PARAMSET[0]);
	for(i=0;i<CSL_EDMA3_TPCC2_NUM_PARAMSETS*8;i++)
	{
		*uipPaRAM++ = 0;
	}
...
/*****************************************************************************
 Prototype    : EDMA_event_enable
 Description  : enable event for EDMA
 Input        : None
 Output       : None
 Return Value :
  ...
  1.Date         : 2010/12/12
    ...
    Modification : Created function
*****************************************************************************/
void EDMA_event_enable(unsigned int uiCC, unsigned int uiChannel)
{
	if(uiChannel < 32)
	{
...
/*****************************************************************************
 Prototype    : EDMA_event_disable
 Description  : disable event for EDMA
 Input        : None
 Output       : None
 Return Value :
  ...
  1.Date         : 2010/12/12
    ...
    Modification : Created function
*****************************************************************************/
void EDMA_event_disable(unsigned int uiCC, unsigned int uiChannel)
{
	if(uiChannel < 32)
	{
...
/*****************************************************************************
 Prototype    : EDMA_interrupt_enable
 Description  : enable interrupt for EDMA
 Input        : None
 Output       : None
 Return Value :
  ...
  1.Date         : 2010/12/12
    ...
    Modification : Created function
*****************************************************************************/
void EDMA_interrupt_enable(unsigned int uiCC, unsigned int uiIntNum)
{
	if(uiIntNum < 32)
	{
...
/*****************************************************************************
 Prototype    : EDMA_interrupt_disable
 Description  : disable interrupt for EDMA
 Input        : None
 Output       : None
 Return Value :
  ...
  1.Date         : 2010/12/12
    ...
    Modification : Created function
*****************************************************************************/
void EDMA_interrupt_disable(unsigned int uiCC, unsigned int uiIntNum)
{
	if(uiIntNum < 32)
	{
...
{
	CSL_TpccRegs *EDMACCRegs;
	unsigned int i, uiChannel, uiChannelShift, uiBCNT;
	volatile unsigned int *TPCC_ESR;
	volatile unsigned int *TPCC_IPR;
	volatile unsigned int *TPCC_ICR;
	unsigned long long tempBuf[128/8];
	unsigned int tempBufSize = 128, headerSize, tailSize;
	...
	if(address & 127)								/* header address not start on 128 byte boundary */
	{
		...
		EDMA_copy((unsigned int)tempBuf, address, headerSize, CC_channel, DMA_WAIT);
		address += headerSize;
	...
	if(byteCount && ((address+byteCount) & 127))	/* tail address not start on 128 byte boundary */
	{
		...
		EDMA_copy((unsigned int)tempBuf, address+byteCount-tailSize, tailSize, CC_channel, DMA_WAIT);
		byteCount -= tailSize;
	}
	...
	while(byteCount)	//the reminding must be multiple of 128 bytes
	{
		...
		*TPCC_ESR = 1 << uiChannelShift;
		...
		/*wait for completion*/
		while((*TPCC_IPR & (1 << uiChannelShift)) == 0);
...
/*****************************************************************************
 ...
 Input        : unsigned int edmaNum, EDMA module number (0~2)
 Output       : None
 Return Value :
...
*****************************************************************************/
void EDMA_CC_error_handler(unsigned int edmaNum)
{
	CSL_TpccRegs *EDMACCRegs = gpEDMA_CC_regs[edmaNum];
	unsigned int i, EMR, EMRH = 0, QEMR, CCERR;
...
{
	unsigned int ERRSTAT, ERRDET, STAT;
...
/*****************************************************************************
 ...
 Return Value :
  ...
  1.Date         : 2012/10/28
    ...
    Modification : Created function
*****************************************************************************/
void EDMA_error_handler(unsigned int edmaNum, unsigned int errorFlag)
{
	unsigned int i;
...
void lock_mem_prot_regs(void)
{
	...
	while((gpCGEM_regs->MPLKSTAT & CSL_CGEM_MPLKSTAT_LK_MASK) == 0);
}
...
void unlock_mem_prot_regs(void)
{
	unsigned int i;
	...
	while((gpCGEM_regs->MPLKSTAT & CSL_CGEM_MPLKSTAT_LK_MASK) == 1);
}

/*****************************************************************************
 Prototype    : L1_MPPA_setup
 Description  : setup the memory protection for local L1 memory
 Input        : volatile unsigned int MPPA_regs[], unsigned int MPPA[]
 Output       : None
 Return Value :

 usage example:
     unsigned int MPPA[16] =
     {
  ...
  1.Date         : 2012/12/6
    ...
    Modification : Created function
*****************************************************************************/
void L1_MPPA_setup(volatile unsigned int MPPA_regs[], unsigned int MPPA[])
{
	unsigned int i;
...
/*****************************************************************************
 ...
 Input        : unsigned int AccessPermisionMask
 Output       : None
 Return Value :
*****************************************************************************/
void L1P_memory_protection_cfg(unsigned int AccessPermisionMask)
{
	unsigned int i;
...
/*****************************************************************************
 ...
 Return Value :
*****************************************************************************/
void L1D_memory_protection_cfg(unsigned int AccessPermisionMask)
{
	unsigned int i;
...
/*****************************************************************************
 ...
 Input        : unsigned int MPPA[]
 Output       : None
 Return Value :
 usage example:
     unsigned int MPPA[32] =
     {
  ...
  1.Date         : 2012/10/5
    ...
    Modification : Created function
*****************************************************************************/
void LL2_MPPA_setup(unsigned int MPPA[])
{
	unsigned int i;
...
/*****************************************************************************
 ...
 Input        : unsigned int startAddress
                unsigned int byteCount
                unsigned int AccessPermisionMask
 Output       : None
 Return Value :
  ...
  1.Date         : 2012/10/5
    ...
    Modification : Created function
*****************************************************************************/
void LL2_Memory_Protection_cfg(unsigned int startAddress,
		unsigned int byteCount, unsigned int AccessPermisionMask)
{
	unsigned int i;
	unsigned int uiEndAddress;
	unsigned int uiPageIndex, uiNumPages, uiPageSize, uiPageAddressMask, uiPageShift;
...
/*****************************************************************************
 ...
 Return Value :
  ...
  1.Date         : 2012/10/5
    ...
    Modification : Created function
*****************************************************************************/
void KeyStone_CorePac_AID_map(unsigned char map_table[])
{
	unsigned int i;
	unsigned int *PAMAP = (unsigned int *)&(gpCGEM_regs->PAMAP0);
...
void KeyStone_MPAX_seg_setup(MPAX_Regs *MPAX_regs, unsigned int BADDR,
		unsigned int RADDR, unsigned int SegementSize, unsigned int AccessPermisionMask)
{
	unsigned int SegSizeMask, SegSizeBits;

	if(SegementSize < 1024*4)
	{
...
/*****************************************************************************
 ...
 Description  : configure multiple MPAX segments in XMC with a configuration
                table.
                Please note, the code of this function or the data
                used by this function (stack, parameters...) can not be put
                in the segment that will be modified by this function.
                Normally, to put this function and its related data in
                LL2 is a good idea.
...
void KeyStone_XMC_MPAX_setup(MPAX_Config MPAX_cfg[],
		unsigned int firstSeg, unsigned int numSegs)
{
	int i;
...
void KeyStone_SMS_MPAX_setup(MPAX_Config MPAX_cfg[],
		unsigned int firstSeg, unsigned int numSegs, unsigned int PrivID)
{
	int i;
...
/*****************************************************************************
 ...
 Input        : unsigned int PRIVID_MASK
 Output       : None
 Return Value :
  ...
  1.Date         : 2012/10/15
    ...
    Modification : Created function
*****************************************************************************/
void KeyStone_MSMC_MP_interrupt_en(unsigned int PRIVID_MASK)
{
...
CSL_MpuRegs *gpMPU_regs[8] =
{
	...
	/* for C6670, TCI6614 */
	(CSL_MpuRegs *)(CSL_MPU_3_REGS+CSL_MPU_1_REGS-CSL_MPU_0_REGS),

	/* for TCI6614 only */
	(CSL_MpuRegs *)(CSL_MPU_3_REGS+(CSL_MPU_1_REGS-CSL_MPU_0_REGS)*2),
	(CSL_MpuRegs *)(CSL_MPU_3_REGS+(CSL_MPU_1_REGS-CSL_MPU_0_REGS)*3),
	(CSL_MpuRegs *)(CSL_MPU_3_REGS+(CSL_MPU_1_REGS-CSL_MPU_0_REGS)*4)
};
...
void KeyStone_MPU_range_setup(CSL_MpuProg_regionRegs *MPPA_regs,
		unsigned int uiStartAddress, unsigned int uiEndAddress, unsigned int AccessPermisionMask)
{
	...
	if((uiStartAddress & boundaryMask) != 0)
	{
...
/*****************************************************************************
 ...
 Input        : unsigned int uiMPU_num: number of the MPU to be setup
                MPU_Range_Config MPU_cfg[]: MPU ranges configuration table
                unsigned int numRangeCfg: number of ranges to be configured
 Output       : None
 Return Value :
  ...
  1.Date         : 2012/12/15
    ...
    Modification : Created function
*****************************************************************************/
void KeyStone_MPU_setup(unsigned int uiMPU_num,
		MPU_Range_Config MPU_cfg[], unsigned int numRangeCfg)
{
	unsigned int i;
	...
	/* enable the interrupt */
	mpuRegs->INT_ENABLE =
			CSL_MPU_INT_ENABLE_ADDR_ERR_EN_MASK |
...
/*****************************************************************************
 ...
 Input        : unsigned int uiMPU_num: number of the MPU to be modifed
                unsigned int uiAddress: address which need modify permision
                unsigned int AccessPermisionMask: permission mask for that range
 Output       : None
 Return Value :
  ...
  1.Date         : 2012/12/13
    ...
    Modification : Created function
*****************************************************************************/
void KeyStone_MPU_MPPA_modify(unsigned int uiMPU_num,
		unsigned int uiAddress, unsigned int AccessPermisionMask)
{
	unsigned int i;
	unsigned int mpuNumProgmableRange;
	CSL_MpuRegs *mpuRegs;
...
/*****************************************************************************
 ...
 Return Value :
  ...
  1.Date         : 2012/10/28
    ...
    Modification : Created function
*****************************************************************************/
void KeyStone_MPU_interrupt_enable(unsigned int uiMPU_num)
{
	...
	gpMPU_regs[uiMPU_num]->INT_ENABLED_STATUS_CLEAR =
			CSL_MPU_INT_ENABLED_STATUS_CLEAR_ENABLED_ADDR_ERR_MASK |
			CSL_MPU_INT_ENABLED_STATUS_CLEAR_ENABLED_PROT_ERR_MASK;
	gpMPU_regs[uiMPU_num]->INT_ENABLE =
			CSL_MPU_INT_ENABLE_ADDR_ERR_EN_MASK |
			CSL_MPU_INT_ENABLE_PROT_ERR_EN_MASK;
...
void KeyStone_MPU_interrupts_enable()
{
			...
			(DSP_Board_Type == TCI6614_EVM))
	{
		for(i=4;i<6;i++)
		{
			...
			if((i == 5) && (TCI6614_EVM== DSP_Board_Type) &&
...
/*****************************************************************************
 ...
 Input        : unsigned int uwMpuNum
 Output       : None
 Return Value :
*****************************************************************************/
void KeyStone_peripherals_MPU_excepiton_handler(unsigned int uwMpuNum)
{
	unsigned int uwStatus;
	unsigned int uwFltAddr;
	unsigned int uwFltStatus;
...
/*****************************************************************************
 ...
 Return Value :
  ...
  1.Date         : 2012/10/15
    ...
    Modification : Created function
*****************************************************************************/
void KeyStone_SL2_EDC_enable(unsigned int scrubCnt)
{
	if(gpMSMC_regs->CFGLCKSTAT & CSL_MSMC_CFGLCKSTAT_WSTAT_MASK)
	{
		CSL_MSMC_unlockNonMPAX();
	...
	while((gpMSMC_regs->SMEDCC & CSL_MSMC_SMEDCC_PRR_MASK) == 0);
	...
	/* the scrubbing engine works every scrubCnt*1024 cycle */
	CSL_MSMC_setCounterBankRefreshRead(scrubCnt);
...
void LL2_EDC_setup(void)
{
	...
	if((DSP_Board_Type == DUAL_NYQUIST_EVM) ||
...
unsigned int uiLL2_scrub_addr = 0x00800000;
/*****************************************************************************
 ...
 Return Value :
  ...
  1.Date         : 2012/10/20
    ...
    Modification : Created function
*****************************************************************************/
void LL2_EDC_scrub(unsigned int uiByteCnt)
{
	unsigned int uiLL2EndAddress = 0x00880000;
	TDSP_Board_Type DSP_Board_Type = KeyStone_Get_dsp_board_type();

	/* size must be multiple of 128 bits and less than 64KB */
	uiByteCnt &= 0xFFF0;

#if 1
	unsigned int i;
	volatile unsigned int *uipAddress = (volatile unsigned int *)uiLL2_scrub_addr;
...
/*****************************************************************************
 ...
 Return Value :
  ...
  2.Date         : 2012/10/30
    Author       : TI
*****************************************************************************/
void L1P_EDC_setup(void)
{
	unsigned int preL1PMPPA[16];
	...
	IDMA_copy(0x00E00000, 0x00E00000, 1024*32, DMA_WAIT);
...
void LL2_EDC_handler(Bool bCorrected)
{
	...
	if(derr | perr | dmaerr | verr)
	{
		...
		if(bCorrected)
		{
			...
			{
				volatile unsigned int *uipAddress = (volatile unsigned int *)(errAddr & 0xFFFFFFC0);
		...
		errStr = (char *)(perr *(unsigned int)LL2_EDC_L1P_err_str +
				derr *(unsigned int)LL2_EDC_L1D_err_str +
				verr *(unsigned int)LL2_EDC_victim_err_str +
				dmaerr *(unsigned int)LL2_EDC_DMA_err_str);

		printf("address 0x%x caused by %s.\n", errAddr, errStr);

		if((gpCGEM_regs->L2EDADDR & CSL_CGEM_L2EDADDR_RAM_MASK) == 0)
		{
...
/*****************************************************************************
 ...
 Return Value :
  ...
  1.Date         : 2012/10/28
    ...
    Modification : Created function
*****************************************************************************/
void L1P_ED_handler(void)
{
	unsigned int errAddr;
	unsigned char errRAM;
...
void KeyStone_SL2_EDC_handler(void)
{
	...
	unsigned short pfeStat;

	/* error address */
	unsigned int errAddr, bitPos, PrivID;
	...
	if((cses == 1) || (ncses == 1))
	{
		if(cses == 1)
		{
			...
			bitPos = (gpMSMC_regs->SMCEA & CSL_MSMC_SMCEA_ESYN_MASK) >> CSL_MSMC_SMCEA_ESYN_SHIFT;
		...
		printf("%d correctable errors, %d non-correctable errors occurred during scrubbing.\n",
				(gpMSMC_regs->SMSECC & CSL_MSMC_SMSECC_SCEC_MASK) >> CSL_MSMC_SMSECC_SCEC_SHIFT,
				(gpMSMC_regs->SMSECC & CSL_MSMC_SMSECC_SNCEC_MASK) >> CSL_MSMC_SMSECC_SNCEC_SHIFT);
	}
	if(cees == 1)
	{
		volatile unsigned int scrubValue;

		errAddr = gpMSMC_regs->SMCERRAR;
		exception_record.status.ext_sts.info.memory.fault_address = errAddr;

		/* scrub the address to correct the error */
		scrubValue= *(volatile unsigned int *)errAddr;		/* touch to cach */
		*(volatile unsigned int *)errAddr = scrubValue;		/* make cache dirty */
...
unsigned int guwIntcCICExcEvtIdx = 95;	/* INTC event number mapping to CIC exception events */
unsigned int gCIC_EXC_out_num = 15;		/* CIC output event number */
unsigned int gCIC_EXC_EN_MASK[8] = {0,0,0,0,0,0,0,0};
/*****************************************************************************
...
void KeyStone_CIC_exception_events_mapping(void)
{
	...
	else if(DSP_Board_Type == C6678_EVM)
	{
		if(DNUM < 4)
		{
...
void KeyStone_Exception_cfg(Bool bGlobalExceptionMaster)
{
	...
	/* only monitor drop of enabled interrupts */
	gpCGEM_regs->INTDMASK= ~IER;
...
{
	unsigned int i;
	unsigned int status[4], status5 = 0;
	TDSP_Board_Type DSP_Board_Type = KeyStone_Get_dsp_board_type();
	...
	if(DSP_Board_Type == TCI6614_EVM)
	{
		...
		if((status[0] & (1 << 23)) || (status[1] & (1 << (37-32))))
		{
			for(i=0;i<8;i++)
			{
				/* MPU5 (for BCP) of TCI6614 is only accessible
				 * when BCP is enabled through PSC */
				if((i==5) && ((CSL_PSC_getPowerDomainState(5) != PSC_PDSTATE_ON) ||
						(CSL_PSC_getModuleState (12) != PSC_MODSTATE_ENABLE)))
						{
							continue;
						}
...
void KeyStone_EXC_external(void)
{
	...
	if(flag[3] & (unsigned int)(1 << (CSL_GEM_EMC_BUSERR-96)))
	{
...
interrupt void Nested_Exception_service_routine(void)
{
	...
	if(exception_record.TSCL == 0)
	{
		exception_record.TSCL = tscl;
	}
	if(exception_record.TSCH == 0)
	{
		exception_record.TSCH= tsch;
	}
	if(exception_record.NRP == 0)
	{
		exception_record.NRP = NRP;
	}
	if(exception_record.NTSR == 0)
	{
		exception_record.NTSR = NTSR;
	}
	if(exception_record.EFR == 0)
	{
		exception_record.EFR = EFR;
	}
	if(exception_record.status.IERR == 0)
	{
		exception_record.status.IERR = IERR;
	}
...
void CPU_interrupt_enable(unsigned int uiIER_en_mask)
{
	ICR = uiIER_en_mask;
	IER |= uiIER_en_mask;
...
void KeyStone_get_device_speed_grade(void)
{
	unsigned int uiDevSpeed, uiSpeedCode;

	uiSpeedCode = DEVSPEED;
...
void KeyStone_get_device_info(void)
{
	char *boot_mode;
	unsigned char ucBootMode, ucBootMaster;
	unsigned int uiVID;
	unsigned char ucPLL_cfg = 8; //8 means bypassed
	unsigned int *DieID = (unsigned int *)&gpBootCfgRegs->DIE_ID_REG0;
	unsigned int uiJTAG_ID = gpBootCfgRegs->DEVICE_ID_REG0;
	unsigned int uiDevStat = gpBootCfgRegs->BOOT_REG0;
	...
	if((gDSP_board_type == TCI6614_EVM)&&(ucBootMaster == 1))
	{
		boot_mode= "ARM";
	}
	else
	...
	printf("DEVSTAT= 0x%08x. %s endian, %s boot, PLL configuration implies the input clock for core is %s.\n",
			uiDevStat, endian_str[uiDevStat & DEVSTAT_LENDIAN_MASK], boot_mode, input_clk_str[ucPLL_cfg]);
...
/*****************************************************************************
 ...
 Description  : common initialization for internal modules in K2 device
  ...
  1.Date         : July 11, 2014
    ...
    Modification : Created function
*****************************************************************************/
void KeyStone_common_device_init(void)
{
	unsigned int i;
